...
class TransferlnStation extends React.Component {
  constructor(props) {
  ...
  getTrades = (page, currency, direction) => {
    axios.get("/kc/wallet/instations", {
        params: {
          direction: direction,
          page: page,
          currency: currency
      ...
      columns = [{
        ...
        render: (text, record, index) => {
          return (
            <p key={index}>
                {
                  this.state.status == 0?<span>转账给{record.to}</span>:<span>{record.from}转入</span>
                }
          </p>
          )
        },
...
TransferlnStation.propTypes = {
  intl: intlShape.isRequired
}
export default injectIntl(TransferlnStation);